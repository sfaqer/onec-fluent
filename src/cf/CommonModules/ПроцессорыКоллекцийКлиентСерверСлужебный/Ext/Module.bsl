...
Процедура ВыполнитьРазвернуть(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДополнительныеПараметры.Коллекция Цикл
		...
		#Если Клиент Тогда
			...
			Если Не ТипЗнч(РезультатРазворачивания) = Тип("ФормаКлиентскогоПриложения") Тогда
				ВызватьИсключение ДСО;
			КонецЕсли;
			
		#Иначе
			
			Если Не ТипЗнч(РезультатРазворачивания) = Тип("ОбработкаОбъект.ПроцессорКоллекций") Тогда
				ВызватьИсключение ДСО;
			КонецЕсли;
...
Процедура ПоложитьЯчейкуВКонвейер(
	...
	Ячейка = ПроцессорыКоллекций.НовыйОписаниеОповещения(
...
Функция СформироватьВременноеОписаниеОповещения(ПользовательскоеВыражение, ДополнительныеПараметры)
	...
	ОписаниеОповещения = ПроцессорыКоллекций.НовыйОписаниеОповещения(
		"ОбработкаОповещения",
		ПроцессорыКоллекцийКлиентСерверСлужебный,
		ДополнительныеПараметры
	);
...
//   * добавлена инициализация структур приемника и источника
//
